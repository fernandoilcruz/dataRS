...
datavisR is an R interface for the [DataVis](https://datavis.dee.rs.gov.br/) API. DataVis is a comprehensive repository of socioeconomic data for the state of Rio Grande do Sul, created and maintained by the Department of Economics and Statistics ([DEE/SPGG-RS](https://dee.rs.gov.br/inicial)) at the Planning, Governance and Management. Besides being extremely intuitive and user-friendly for data downloads, the implementation of an API turned this task even faster and more efficient. Data analysts interested in the state's multiple sets of socioeconomic data have now a more appropriate tool to work with. R users, in particular, can benefit from this using datavisR, an R package that provides an easy way to access the DataVis API.
